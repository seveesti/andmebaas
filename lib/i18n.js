...
function parseHeaderMenu(menu) {
	...
	menu.forEach(function(obj) {
		var item = parse(obj)
		var parentId = obj.menu_item_parent

		// Presuming parents come before children.
		if (!parentId || parentId == "0") itemsById[obj.ID] = item
		else if (itemsById[parentId]) itemsById[parentId].children.push(item)
	})

	return _.values(itemsById)
...
exports.updateMenus = function() {
	async function get(lang) {
		var header = (await request("/wp-json/wp/v2/menu?lang=" + lang)).body
